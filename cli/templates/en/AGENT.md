...
### 5.1 Long Conversation Handling

When conversation exceeds 10 turns or involves multiple files:

```
1. Proactively summarize progress:
   "So far: completed [A, B], in progress [C], pending [D]"

2. Identify context that can be "closed":
   - Fixed errors → stop repeating details
   - Confirmed decisions → execute directly, don't re-ask

3. If user starts a new topic:
   "Is this a new task or related to the previous [X]?"
```

### 5.2 When to Suggest Reset

```
Suggest starting a new conversation if:
- Completely different project/feature
- Previous errors resolved, starting fresh development
- Conversation becomes confusing or repetitive
```

### 5.3 Information Handoff

When work needs to continue in a new conversation, provide:

```
"If you start a new conversation, begin with:
'Continuing [project name], last completed [X], now doing [Y].
Related files: [list]'"
```
